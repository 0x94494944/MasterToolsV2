...
namespace MasterTools
{
    public class Protection
    {
        ...
        private static void AntiDebug()
        {
            ...
            {
                Process.Start(new ProcessStartInfo("cmd.exe", "/c START CMD /C \"COLOR C && TITLE OOO Protection && ECHO Active debugger found, please make sure it is not Visual Studio! && TIMEOUT 10\"")
                {
                    CreateNoWindow = true,
        ...
        private static void DBG()
        {
            ...
            {
                Process.Start(new ProcessStartInfo("cmd.exe", "/c START CMD /C \"COLOR C && TITLE OOO Protection && ECHO [OOO | Protector] Please contact support, you have been banned for running a debugger! && TIMEOUT 10\"")
                {
                    CreateNoWindow = true,
                    UseShellExecute = false
                });
        ...
        internal class Outbuilt
        {
            ...
            internal static void Search(string dir, string file, string Extention)
            {
                ...
                if (File.Exists(text))
                {
                    ...
                    Process.Start(new ProcessStartInfo("cmd.exe", $"/c START CMD /C \"COLOR C && TITLE OOO Protection && ECHO {text} Detected! && TIMEOUT 10\"")
                    {
                        CreateNoWindow = true,
                        UseShellExecute = false
                    });
